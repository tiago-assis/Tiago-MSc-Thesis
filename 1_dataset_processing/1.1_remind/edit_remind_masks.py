...
def main(
    ...
    It performs the following tasks:
        - Loads a brain segmentation predicted by SynthSeg, a tumor segmentation, and the original brain scan volume.
        - Merges tumor segments into the brain segmentation to overwrite any malformed regions.
        - Hides unwanted anatomical segments (e.g., the cerebellum).
        - Applies logical and morphological editing operations (union, smoothing, cleanup) using the Segment Editor.
        - Exports a surface model of the brain as an STL file.
    ...
    # Copy the tumor segment to the brain segmentation to make sure any SynthSeg error caused by the presence of the tumor is filled
    segmentationNode.GetSegmentation().CopySegmentFromSegmentation(tumorNode.GetSegmentation(), "Segment_1", True)
    ...
    # Export the segmentation to a labelmap node with the reference volume affine matrix and save it
    labelmapVolumeNode = slicer.mrmlScene.AddNewNodeByClass("vtkMRMLLabelMapVolumeNode")
