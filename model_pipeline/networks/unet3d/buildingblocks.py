...
def create_conv(
    ...
    for i, char in enumerate(order):
        if char == "r":
            ...
            if is_before_conv:
                modules.append(("instancenorm", inorm(in_channels)))
            else:
                modules.append(("instancenorm", inorm(out_channels)))
        else:
            raise ValueError(
                f"Unsupported layer type '{char}'. MUST be one of ['b', 'g', 'i', 'r', 'l', 'e', 'c']"
            )
...
class ResNetBlock(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if "l" in order:
            self.non_linearity = nn.LeakyReLU(negative_slope=0.01, inplace=True)
        elif "e" in order:
